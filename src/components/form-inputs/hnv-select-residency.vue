  ...
  <Listbox as="div" v-model="selected">
    ...
    <div class="mt-1 relative">
      <ListboxButton class="relative w-full bg-white border border-gray-300 rounded-md shadow-sm pl-3 pr-10 py-2 text-left cursor-default focus:outline-none focus:ring-1 focus:ring-olive-500 focus:border-olive-500 sm:text-sm">
        <span :class="[selected ? 'capitalize' : 'text-gray-500', 'block truncate']">{{ selected || 'Select a Residency...' }}</span>
      ...
      <transition leave-active-class="transition ease-in duration-100" leave-from-class="opacity-100" leave-to-class="opacity-0">
        <ListboxOptions class="absolute z-10 mt-1 w-full bg-white shadow-lg max-h-60 rounded-md py-1 text-base ring-1 ring-black ring-opacity-5 overflow-auto focus:outline-none sm:text-sm">
          <ListboxOption as="template" v-for="residency in residencyOptions" :key="residency" :value="residency" v-slot="{ active, selected }">
...
export default {
  ...
  setup (props, { emit }) {
    ...
    const selectedOption = isEmptyModelValue ? '' : residencyOptions.find((item) => item.id === props.modelValue)
