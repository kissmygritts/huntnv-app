...
<template>
  <Listbox as="div" v-model="selected">
    ...
    <div class="mt-1 relative">
      <ListboxButton
        class="relative w-full bg-white border border-gray-300 rounded-md shadow-sm pl-3 pr-10 py-2 text-left cursor-default focus:outline-none focus:ring-1 focus:ring-saffron-500 focus:border-saffron-500 sm:text-sm"
      >
        <span
          :class="[
            selected.id ? 'capitalize' : 'text-gray-500',
            'block truncate'
          ]"
          >{{
            selected.id ? selected.displayName : 'Select a Species...'
          }}</span
        >
        <span
      ...
      >
        ...
        >
          ...
          >
            <li
              :class="[
                active ? 'text-saffron-700 bg-saffron-200/50' : 'text-gray-900',
            ...
            >
              ...
              <span
                v-if="selected"
                :class="[
                  active ? 'text-saffron-700' : 'text-saffron-700',
...
export default {
  ...
  setup(props, { emit }) {
    ...
    const selectedOption = isEmptyModelValue
      ? {}
      : speciesOptions.find((item) => item.id === props.modelValue.id)
